...
// 18. 单调栈：
// 单调栈（Monotonic Stack）是一种常用的数据结构，主要用于解决一些与区间、最值等相关的问题。它的主要作用是在处理一维数组或者字符串时，快速找到当前元素的某种性质满足的前后边界或者一些特定区间的最值等信息。

// 单调栈通常用于解决以下类型的问题：

// 找到某个元素的前后边界：单调栈可以帮助我们找到数组中每个元素的左边或右边第一个比它大或比它小的元素的位置。这类问题通常可以使用单调递增栈或单调递减栈来解决。
// 找到数组中的区间最值：单调栈可以帮助我们在线性时间内找到数组中每个元素所在的区间内的最值。通过维护一个单调递增栈或者单调递减栈，我们可以快速找到每个元素的左边或右边第一个比它小或大的元素的位置，从而计算出该元素所在区间的最值。
// 解决连续子数组的问题：对于某些连续子数组的问题，可以使用单调栈来优化求解过程。例如，最大矩形面积、柱状图中的最大矩形等问题，都可以使用单调栈来解决。